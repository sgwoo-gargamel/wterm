...
- **워크스페이스** — 왼쪽 위 이름표가 현재 워크스페이스 이름입니다. 옆의 ⌄ 를 누르면 현재 타일 배치와
  각 타일의 연결을 그 이름으로 저장하고, 목록에서 골라 그대로 다시 열 수 있습니다(불러오면 저장된
  연결로 자동 접속하며, SSH는 키 인증에 실패하면 비밀번호 입력 폼이 뜹니다). 항목별 삭제를 지원합니다
...
- **멀티 입력** — 여러 타일을 선택해 같은 명령을 한 번에 전송
- **입력창** — 툴바(멀티 입력)와 각 타일 타이틀 바에 같은 형태의 입력창이 있습니다. 클릭하면
  큰 입력창과 이력 목록이 함께 열리고, ↑/↓ 키로 이전 입력을 꺼내 쓸 수 있습니다.
  이력은 툴바와 모든 타일이 함께 쓰며(전역), 항목별 삭제와 전체 삭제를 지원합니다
...
모든 설정(언어·테마·색상·폰트·로그 폴더·연결 이력·입력 이력·워크스페이스)이
`wterm-settings.json` 한 파일에 저장됩니다.
