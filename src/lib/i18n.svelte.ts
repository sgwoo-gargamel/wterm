import { getSetting, setSetting } from './persist';

export type Locale = 'ko' | 'en';

const ko = {
	'group.addTile': '창 추가',
	'group.logDir': '로그 폴더',
	'group.settings': '설정',
	'toolbar.addLeft': '왼쪽에 타일 추가',
	'toolbar.addRight': '오른쪽에 타일 추가',
	'toolbar.addUp': '위에 타일 추가',
	'toolbar.addDown': '아래에 타일 추가',
	'toolbar.close': '닫기',
	'toolbar.theme': '테마 전환',
	'pane.empty': '새 연결',
	'pane.splitRight': '좌우 분할',
	'pane.splitDown': '상하 분할',
	'pane.maximize': '확대',
	'pane.restore': '축소',
	'pane.sendSize': '터미널 크기 전송 (stty) — 타겟이 셸 프롬프트일 때 사용',
	'pane.clear': '화면 지우기 (버퍼 클리어)',
	'xmodem.send': '파일 전송 (XMODEM) — 타겟에서 수신(rx, loadx)을 먼저 실행하세요',
	'xmodem.waitingHint': '타겟에서 rx 또는 loadx가 실행 중이어야 합니다',
	'ymodem.send': '파일 전송 (YMODEM) — 타겟에서 수신(rb, loady)을 먼저 실행하세요',
	'ymodem.waitingHint': '타겟에서 rb 또는 loady가 실행 중이어야 합니다',
	'transfer.waiting': '수신 응답 대기 중…',
	'transfer.sending': '전송 중…',
	'transfer.done': '전송 완료',
	'transfer.failed': '전송 실패',
	'toolbar.logDir': '저장 폴더 설정',
	'multi.placeholder': '멀티 입력',
	'multi.send': '전송',
	'multi.all': '전체',
	'multi.target': '멀티 입력 받기',
	'multi.label': '멀티',
	'workspace.menu': '워크스페이스',
	'toolbar.about': '정보',
	'app.version': '버전',
	'app.revision': '리비전',
	'workspace.name': '워크스페이스 이름 (클릭해서 수정)',
	'workspace.save': '현재 상태 저장',
	'workspace.saved': '저장된 워크스페이스',
	'workspace.empty': '저장된 워크스페이스가 없습니다',
	'workspace.remove': '삭제',
	'workspace.hint': '타일 배치와 각 타일의 연결을 저장했다가 그대로 다시 엽니다',
	'workspace.needName': '저장하려면 왼쪽 위 이름을 먼저 바꾸세요',
	'workspace.new': '새로 시작',
	'workspace.newHint': '열린 연결을 모두 닫고 빈 창 하나로 되돌립니다',
	'send.placeholder': '입력',
	'multi.historyRemove': '이력에서 삭제',
	'multi.historyClear': '전체 삭제',
	'log.title': '로그 저장',
	'log.fileName': '파일 이름',
	'log.timestamp': '각 줄에 수신 시각 기록',
	'log.plain': '제어 문자 제거 (순수 텍스트)',
	'log.start': '저장 시작',
	'log.stop': '저장 중지',
	'log.saving': '저장 중',
	'log.noDir': '저장 폴더를 먼저 설정하세요',
	'toolbar.settings': '폰트 설정',
	'window.minimize': '최소화',
	'window.maximize': '최대화/복원',
	'window.close': '닫기',
	'settings.fontFamily': '터미널 폰트',
	'settings.fontSize': '폰트 크기',
	'settings.reset': '폰트 초기화',
	'settings.colors': '색상',
	'settings.colorBg': '배경',
	'settings.colorFg': '글자',
	'settings.colorCursor': '커서',
	'settings.colorAccent': '강조',
	'settings.resetColors': '색상 초기화',
	'color.theme': '테마 색',
	'color.standard': '표준 색',
	'color.custom': '다른 색…',
	'settings.fontWeight': '굵기',
	'settings.weight.light': '가늘게',
	'settings.weight.normal': '보통',
	'settings.weight.medium': '중간',
	'settings.weight.bold': '굵게',
	'settings.renderer': '렌더링',
	'settings.renderer.webgl': '빠르게 (WebGL)',
	'settings.renderer.dom': '선명하게 (DOM)',
	'tab.serial': '시리얼',
	'tab.local': '로컬',
	'form.shell': '셸',
	'form.noDistros': '사용 가능한 셸 없음',
	'form.startDir': '시작 위치',
	'form.startDirHome': '기본: 사용자 홈',
	'form.startDirWsl': '기본: 배포판 홈',
	'form.startDirVenv': '.venv가 있는 프로젝트 폴더 (필수)',
	'form.venvMissing': '이 폴더에 .venv가 없습니다. .venv\\Scripts\\Activate.ps1이 있는 폴더를 선택하세요',
	'form.browse': '폴더 선택',
	'form.port': '포트',
	'form.baudRate': 'Baud rate',
	'form.host': '호스트 (IP)',
	'form.username': '사용자 ID',
	'form.usernameOptional': ' (선택, 프롬프트 자동 입력)',
	'form.password': '비밀번호 (SSH 키 인증 실패 시 사용, 저장되지 않음)',
	'error.passwordRequired': '사용 가능한 SSH 키가 없습니다. 비밀번호를 입력하세요',
	'error.invalidCwd': '시작 위치 폴더를 찾을 수 없습니다',
	'form.connect': '연결',
	'form.connecting': '연결 중…',
	'form.noPorts': '포트 없음',
	'form.loadingPorts': '포트 검색 중…',
	'form.scanning': '장치 검색 중…',
	'form.scanDone': '검색 완료',
	'form.portInUse': '사용 중',
	'form.portMissing': '연결된 포트가 아닙니다. 케이블을 확인하고 ↻를 누르세요',
	'form.portBusy': '다른 프로그램이 사용 중입니다',
	'form.refreshPorts': '포트 새로고침',
	'progress.cancelHint': '누르면 취소',
	'progress.failed': '연결 실패',
	'progress.closeHint': '누르면 닫기',
	'progress.cancelled': '연결을 취소했습니다',
	'history.title': '최근 연결',
	'history.empty': '저장된 연결 이력이 없습니다',
	'history.remove': '이력에서 삭제',
	'overlay.reconnect': '다시 연결',
	'overlay.newConnection': '새 연결',
	'overlay.closeTile': '닫기',
	'reason.closed': '연결이 종료되었습니다',
	'reason.remote-closed': '원격 호스트가 연결을 종료했습니다',
	'reason.write-failed': '전송 실패: 연결이 끊어졌습니다',
	'reason.device-removed': '장치가 분리되었습니다',
	'reason.cancelled': '전송을 취소했습니다',
	'reason.remote-cancelled': '수신 측에서 전송을 취소했습니다',
	'reason.handshake-timeout': '수신 응답이 없습니다',
	'reason.transfer-timeout': '전송 중 수신 응답이 끊어졌습니다',
	'save.failed': '설정을 저장하지 못했습니다',
	'save.failedHint': '변경한 내용이 다음 실행에 남지 않습니다. 아래 경로에 쓰기 권한이 있는지 확인하세요.',
	'save.dismiss': '닫기'
};

const en: Record<MessageKey, string> = {
	'group.addTile': 'Add pane',
	'group.logDir': 'Log folder',
	'group.settings': 'Settings',
	'toolbar.addLeft': 'Add tile left',
	'toolbar.addRight': 'Add tile right',
	'toolbar.addUp': 'Add tile above',
	'toolbar.addDown': 'Add tile below',
	'toolbar.close': 'Close',
	'toolbar.theme': 'Toggle theme',
	'pane.empty': 'New connection',
	'pane.splitRight': 'Split right',
	'pane.splitDown': 'Split down',
	'pane.maximize': 'Maximize',
	'pane.restore': 'Restore',
	'pane.sendSize': 'Send terminal size (stty) — use at a shell prompt',
	'pane.clear': 'Clear screen (buffer)',
	'xmodem.send': 'Send file (XMODEM) — start the receiver (rx, loadx) on the target first',
	'xmodem.waitingHint': 'rx or loadx must be running on the target',
	'ymodem.send': 'Send file (YMODEM) — start the receiver (rb, loady) on the target first',
	'ymodem.waitingHint': 'rb or loady must be running on the target',
	'transfer.waiting': 'Waiting for receiver…',
	'transfer.sending': 'Sending…',
	'transfer.done': 'Transfer complete',
	'transfer.failed': 'Transfer failed',
	'toolbar.logDir': 'Log folder',
	'multi.placeholder': 'Multi-send',
	'multi.send': 'Send',
	'multi.all': 'All',
	'multi.target': 'Receive multi-send',
	'multi.label': 'Multi',
	'workspace.menu': 'Workspaces',
	'toolbar.about': 'About',
	'app.version': 'Version',
	'app.revision': 'Revision',
	'workspace.name': 'Workspace name (click to edit)',
	'workspace.save': 'Save current layout',
	'workspace.saved': 'Saved workspaces',
	'workspace.empty': 'No saved workspaces',
	'workspace.remove': 'Delete',
	'workspace.hint': 'Stores the tile layout and each tile’s connection, and reopens them as they were',
	'workspace.needName': 'Rename it in the top left before saving',
	'workspace.new': 'Start fresh',
	'workspace.newHint': 'Closes every open connection and returns to a single empty tile',
	'send.placeholder': 'Send line',
	'multi.historyRemove': 'Remove from history',
	'multi.historyClear': 'Clear all',
	'log.title': 'Save log',
	'log.fileName': 'File name',
	'log.timestamp': 'Timestamp each line',
	'log.plain': 'Strip control codes (plain text)',
	'log.start': 'Start saving',
	'log.stop': 'Stop saving',
	'log.saving': 'Saving',
	'log.noDir': 'Set a log folder first',
	'toolbar.settings': 'Font settings',
	'window.minimize': 'Minimize',
	'window.maximize': 'Maximize/restore',
	'window.close': 'Close',
	'settings.fontFamily': 'Terminal font',
	'settings.fontSize': 'Font size',
	'settings.reset': 'Reset font',
	'settings.colors': 'Colors',
	'settings.colorBg': 'Background',
	'settings.colorFg': 'Text',
	'settings.colorCursor': 'Cursor',
	'settings.colorAccent': 'Accent',
	'settings.resetColors': 'Reset colors',
	'color.theme': 'Theme colors',
	'color.standard': 'Standard colors',
	'color.custom': 'More colors…',
	'settings.fontWeight': 'Weight',
	'settings.weight.light': 'Light',
	'settings.weight.normal': 'Normal',
	'settings.weight.medium': 'Medium',
	'settings.weight.bold': 'Bold',
	'settings.renderer': 'Rendering',
	'settings.renderer.webgl': 'Fast (WebGL)',
	'settings.renderer.dom': 'Sharp (DOM)',
	'tab.serial': 'Serial',
	'tab.local': 'Local',
	'form.shell': 'Shell',
	'form.noDistros': 'No shells available',
	'form.startDir': 'Start directory',
	'form.startDirHome': 'Default: user home',
	'form.startDirWsl': 'Default: distro home',
	'form.startDirVenv': 'Project folder containing .venv (required)',
	'form.venvMissing': 'No .venv in this folder. Pick a folder containing .venv\\Scripts\\Activate.ps1',
	'form.browse': 'Browse',
	'form.port': 'Port',
	'form.baudRate': 'Baud rate',
	'form.host': 'Host (IP)',
	'form.username': 'Username',
	'form.usernameOptional': ' (optional, auto-entered at prompt)',
	'form.password': 'Password (used if key auth fails; not saved)',
	'error.passwordRequired': 'No usable SSH key. Enter a password',
	'error.invalidCwd': 'Start directory not found',
	'form.connect': 'Connect',
	'form.connecting': 'Connecting…',
	'form.noPorts': 'No ports',
	'form.loadingPorts': 'Scanning ports…',
	'form.scanning': 'Scanning devices…',
	'form.scanDone': 'Scan complete',
	'form.portInUse': 'in use',
	'form.portMissing': 'Port not connected. Check the cable and press ↻',
	'form.portBusy': 'In use by another program',
	'form.refreshPorts': 'Refresh ports',
	'progress.cancelHint': 'Click to cancel',
	'progress.failed': 'Connection failed',
	'progress.closeHint': 'Click to dismiss',
	'progress.cancelled': 'Connection cancelled',
	'history.title': 'Recent connections',
	'history.empty': 'No saved connection history',
	'history.remove': 'Remove from history',
	'overlay.reconnect': 'Reconnect',
	'overlay.newConnection': 'New connection',
	'overlay.closeTile': 'Close',
	'reason.closed': 'Connection closed',
	'reason.remote-closed': 'The remote host closed the connection',
	'reason.write-failed': 'Send failed: connection lost',
	'reason.device-removed': 'The device was disconnected',
	'reason.cancelled': 'Transfer cancelled',
	'reason.remote-cancelled': 'The receiver cancelled the transfer',
	'reason.handshake-timeout': 'No response from the receiver',
	'reason.transfer-timeout': 'The receiver stopped responding mid-transfer',
	'save.failed': 'Could not save settings',
	'save.failedHint': 'Your changes will not survive a restart. Check that this location is writable.',
	'save.dismiss': 'Dismiss'
};

export type MessageKey = keyof typeof ko;

const messages: Record<Locale, Record<MessageKey, string>> = { ko, en };

export const i18n = $state<{ locale: Locale }>({ locale: 'ko' });

export function initLocale() {
	const saved = getSetting('locale');
	if (saved === 'ko' || saved === 'en') i18n.locale = saved;
}

export function setLocale(locale: Locale) {
	i18n.locale = locale;
	setSetting('locale', locale);
}

export function t(key: MessageKey): string {
	return messages[i18n.locale][key] ?? key;
}

/** Backend sends stable tokens (closed, remote-closed, ...); raw error text passes through as-is */
export function translateReason(reason: string): string {
	const key = `reason.${reason}` as MessageKey;
	const table = messages[i18n.locale];
	return key in table ? table[key] : reason;
}
